...
def ForwardModelsVal(args, task_cfg, device, task_id, batch, model, task_losses):
    ...
    if task_id in ['TASK1', 'TASK2', 'TASK5', 'TASK6', 'TASK7']:
        ...
        features = features.unsqueeze(1).expand(batch_size, num_options, max_num_bbox, 2048).contiguous().view(-1,
                                                                                                               max_num_bbox,
                                                                                                               2048)
        spatials = spatials.unsqueeze(1).expand(batch_size, num_options, max_num_bbox, 5).contiguous().view(-1,
                                                                                                            max_num_bbox,
                                                                                                            5)
        image_mask = image_mask.unsqueeze(1).expand(batch_size, num_options, max_num_bbox).contiguous().view(-1,
                                                                                                             max_num_bbox)
        ...
        num_options = question.size(1)
        features = features.expand(batch_size, num_options, max_num_bbox, 2048).contiguous().view(-1, max_num_bbox,
                                                                                                  2048)
    ...
    vil_prediction, vil_logit, vil_binary_prediction, vision_prediction, vision_logit, linguisic_prediction, linguisic_logit = \
        model(question, features, spatials, segment_ids, input_mask, image_mask, co_attention_mask,training=False)
...
def ForwardModelsTrain(args, task_cfg, device, task_id, task_count, task_iter_train, task_dataloader_train, model,
                       task_losses, task_start_iter):
    ...
    # Nathan, to not need apex (hard to install on VSC)
    return model(question, features, spatials, segment_ids, input_mask, image_mask, co_attention_mask,
                 task_cfg={k:dict(v) for k,v in task_cfg.items()}, # Torch doesn't play nice with EasyDict
                 task_id=task_id,
...
def EvaluatingModel(args, task_cfg, device, task_id, batch, model, task_dataloader, task_losses, results, others):
    ...
    if task_cfg[task_id]['type'] == 'VL-classifier':
        ...
        for i in range(select_idx.size(0)):
            results.append(
                {'id': question_id[i].item(), 'target': select_idx[i].item(), 'IOU': select_target[i].item()})
