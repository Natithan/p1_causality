...
def _file_name(row):
    if not os.path.exists(row['folder']): # Nathan
...
def download_image(row):
    ...
    try:
        ...
        timeout = 2 #Nathan, og. 10
        response = requests.get(row['url'], stream=False, timeout=timeout, allow_redirects=True, headers=headers)
        row['status'] = response.status_code
        #row['headers'] = dict(response.headers)
...
def open_tsv(fname, folder):
    ...
    # df = pd.read_csv(fname, sep='\t', names=["caption","url"], usecols=range(1,2),nrows=200) # Nathan added
    df['folder'] = folder
...
if __name__ == "__main__":
    ...
    num_processes = 32
    # num_processes = 1 # Nathan
    # chunk_size is how many images per chunk per process - changing this resets progress when restarting.
    images_per_part = 100
    DATA_ROOT_FOLDER = '/cw/liir/NoCsBack/testliir/datasets/ConceptualCaptions'
    data_name = "validation"
    df = open_tsv("Validation_GCC-1.1.0-Validation.tsv", os.path.join(DATA_ROOT_FOLDER,data_name))
    ...
    data_name = "training"
    df = open_tsv("Train_GCC-training.tsv",os.path.join(DATA_ROOT_FOLDER,data_name))
    df_multiprocess(df=df, processes=num_processes, chunk_size=images_per_part, func=download_image, dataset_name=data_name)
    df = df_from_shelve(chunk_size=images_per_part, func=download_image, dataset_name=data_name)
    df.to_csv("downloaded_%s_report.tsv.gz" % data_name, compression='gzip', sep='\t', header=False, index=False)
    print("Saved.")
